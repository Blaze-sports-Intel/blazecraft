...
const UPGRADE_PATHS = {
    ...
    townhall: ['townhall', 'tower', 'stables'], // HQ upgrades
};
...
export function recordRegionActivity(regionId, taskValue = 1) {
    ...
    if (!region)
        return { upgraded: false, newType: null, level: 0 };
    ...
    if (region.tasksCompleted >= nextThreshold && region.level < maxLevel) {
        ...
        if (newType && newType !== region.type) {
            ...
            return { upgraded: true, newType, level: region.level };
        }
    }
    ...
    return { upgraded: false, newType: null, level: region.level };
}
/**
...
function getBaseType(type) {
    const entries = Object.entries(UPGRADE_PATHS);
